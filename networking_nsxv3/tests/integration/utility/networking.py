...
def create_subnet(con, net_name, subnet_name, net_subnet_range):
    ...
    if subnet == None:
        con.network.create_subnet(network_name_or_id=net_name, subnet_name=subnet_name, cidr=net_subnet_range,
                                  ip_version=4)
    else:
        LOG.debug(f"subnet {subnet_name} is already there")
...
def create_network(con, net_name):
    ...
    if net == None:
        ...
        LOG.debug(f"network {net_name} is already there")
...
def create_port(con, port_name, net_name):
    ...
    if port == None:
        ...
        LOG.debug(f"port {port_name} is already there")
...
def create_trunk(con, trunk_config):
    ...
    if trunk == None and not parent_port == None:
        con.network.create_trunk(name=trunk_name,
...
def dettatch_subport_from_trunk(con, trunk_config):
    trunk_name = trunk_config.get("NAME")
    segmentation_id = trunk_config.get("SEGMENTATION_ID")
    child_port_name = trunk_config.get("CHILD_PORT")
    segmentation_type = "vlan"
    ...
    if not trunk == None and not child_port == None:
        ...
        con.network.delete_trunk_subports(trunk, subports)
    else:
        LOG.debug(f"Could not unset child_port from parent")
...
def attatch_subport_to_trunk(con, trunk_config):
    ...
    if not trunk == None and not child_port == None and not segmentation_id == None:
        LOG.debug(f"add subport {child_port.name} to trunk {trunk_name}")
        subports = [
            {
                "segmentation_id": segmentation_id,
                "port_id": child_port.id,
        ...
        LOG.debug("subport %s not added to trunk %s " % (child_port_name, trunk_name))
...
def delete_trunk(con, trunk_config):
    ...
    if not trunk == None:
        ...
        LOG.debug("no trunk to clean up")


def find_ports_by_server_name(con, server_cfg):
    ports = []
    server = con.compute.find_server(server_cfg.get('NAME'))
